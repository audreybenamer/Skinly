...
public class EditRoutineAdapterPM extends RealmRecyclerViewAdapter<ProductsClass, EditRoutineAdapterPM.ViewHolder> {

    // IMPORTANT
    // THE CONTAINING ACTIVITY NEEDS TO BE PASSED SO YOU CAN GET THE LayoutInflator(see below)
    EditDailyScreen activity;

    public EditRoutineAdapterPM(EditDailyScreen activity, @Nullable OrderedRealmCollection<ProductsClass> data, boolean autoUpdate) {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        List<String> checkedpm = activity.productscheckedpm;

        // copy all the values needed to the appropriate views
        holder.routinecheck.setText(p.getProductname());
        if(checkedpm.contains(p.getProductname())){
            holder.routinecheck.setChecked(true);
        }
        ...
        holder.routinecheck.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if(isChecked){
                    activity.checkproductPM((String) buttonView.getTag());
                }
                else{
                    activity.uncheckproductPM((String) buttonView.getTag());
                }
            }
        });
